...
Select Case Left(sData, 1)
    ...
    Select Case Left$(sData, 2)
        Case "AS"
            ...
            frmMain.ExpShp.width = (((UserExp / 100) / (UserPasarNivel / 100)) * 120)
            ...
            frmMain.HamBar.Caption = UserMinHAM & "/100"
            frmMain.AguBar.Caption = UserMinAGU & "/100"
        ...
        Case "CM"              ' >>>>> Cargar Mapa :: CM
            ...
            If FileExist(App.path & "\MAPAS\" & "Mapa" & UserMap & ".map", vbNormal) Then
                Open App.path & "\MAPAS\" & "Mapa" & UserMap & ".map" For Binary As #1
                Seek #1, 1
                Get #1, , tempint
                Close #1
...
Exit Sub
    ...
    Case "CLA" ' Crea Aura Sobre El Char
Rdata = Right$(Rdata, Len(Rdata) - 3)
CharIndex = Val(ReadField(1, Rdata, 44))
charlist(CharIndex).Aura_Index = Val(ReadField(2, Rdata, 44))
Call InitGrh(charlist(CharIndex).Aura, Val(ReadField(2, Rdata, 44)))
    
   Case "CLT" ' Crea Aura Sobre El Char
Rdata = Right$(Rdata, Len(Rdata) - 3)
CharIndex = Val(ReadField(1, Rdata, 44))
charlist(CharIndex).Aurt_Index = Val(ReadField(2, Rdata, 44))
Call InitGrh(charlist(CharIndex).Aurt, Val(ReadField(2, Rdata, 44)))
        Case "CLU" ' Crea Aura Sobre El Char
Rdata = Right$(Rdata, Len(Rdata) - 3)
CharIndex = Val(ReadField(1, Rdata, 44))
charlist(CharIndex).Auro_Index = Val(ReadField(2, Rdata, 44))
Call InitGrh(charlist(CharIndex).Auro, Val(ReadField(2, Rdata, 44)))
Case "CLO" ' Crea Aura Sobre El Char
Rdata = Right$(Rdata, Len(Rdata) - 3)
CharIndex = Val(ReadField(1, Rdata, 44))
charlist(CharIndex).Aure_Index = Val(ReadField(2, Rdata, 44))
Call InitGrh(charlist(CharIndex).Aure, Val(ReadField(2, Rdata, 44)))
...
#End If
    ...
    Select Case Left$(sData, 4)
        ...
        Case "EHYS"              ' Actualiza Hambre y Sed :: EHYS
            ...
            frmMain.AGUAsp.width = (((UserMinAGU / 100) / (UserMaxAGU / 100)) * 100)
            frmMain.COMIDAsp.width = (((UserMinHAM / 100) / (UserMaxHAM / 100)) * 100)
        ...
        Case "SUNI"             ' >>>>> Subir Nivel :: SUNI
            ...
            frmEstadisticas.Command2.Visible = True
            Exit Sub
