...
Procesado = True 'ver al final del sub
    ...
    Select Case UCase$(Left$(rData, 1))
            ...
            Call SendData(SendTarget.ToIndex, Usuario, 0, "||Tu consulta ah sido ignorada, Por favor relee el Reglamento." & "~255~255~255~1~0")
...
If UserList(UserIndex).flags.Muerto = 1 Then
            ...
            If UserList(UserIndex).flags.Privilegios > User Then
                ...
                Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "||" & UserList(UserIndex).name & "> " & rData & "~0~255~0~1~0")
                End If
            ElseIf UserList(UserIndex).flags.Privilegios = User Then
                ...
                If Not rData = vbNullString Then
                Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "||" & UserList(UserIndex).name & "> " & rData & "~255~255~255~0~0")
                End If
            End If
            End If
            Exit Sub
        ...
        Call SendData(SendTarget.ToAll, 0, 0, "||" & UserList(UserIndex).name & "> " & rData & FONTTYPE_GLOBAL)
           ...
           If UserList(UserIndex).flags.Muerto = 1 Then
            ...
            If tIndex <> 0 Then
                ...
                If EstaPCarea(UserIndex, tIndex) Then
                    Call SendData(SendTarget.ToIndex, UserIndex, UserList(UserIndex).Pos.Map, "||" & RGB(255, 215, 55) & "°" & tMessage & "°" & str(ind))
                    Call SendData(SendTarget.ToIndex, tIndex, UserList(UserIndex).Pos.Map, "||" & RGB(255, 215, 55) & "°" & tMessage & "°" & str(ind))
                End If
                   
                Call SendData(SendTarget.ToIndex, UserIndex, 0, "||" & UserList(UserIndex).name & ": " & tMessage & FONTTYPE_PRIVADO)
                Call SendData(SendTarget.ToIndex, tIndex, 0, "||" & UserList(UserIndex).name & ": " & tMessage & FONTTYPE_PRIVADO)
    ...
    '        Exit Sub
        ...
        Case "UK"
            ...
            Select Case val(rData)
               ...
               'Mithrandir
                Case Magia
                    If Hechizos(UserList(UserIndex).Stats.UserHechizos(UserList(UserIndex).flags.Hechizo)).Tipo = uAutolanzar Then
                        Call HechizoAutolanzable(UserIndex)
                    Else
