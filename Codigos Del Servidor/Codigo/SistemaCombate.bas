...
'Plus golpe certero
If UserList(UserIndex).flags.EfectoTiene = 1 Then
    ...
    UserList(UserIndex).flags.EfectoTiene = 0
Else
...
Sub UsuarioAtacadoPorUsuario(ByVal AttackerIndex As Integer, ByVal VictimIndex As Integer)
    ...
    If UserList(AttackerIndex).Invent.HerramientaEqpObjIndex = 874 And RandomNumber(1, 10) < 3 Then
        UserList(VictimIndex).flags.Paralizado = 1
        UserList(VictimIndex).Counters.Paralisis = IntervaloParalizado
        Call SendData(SendTarget.ToIndex, VictimIndex, 0, "||" & UserList(AttackerIndex).name & " te paralizó con su golpe" & FONTTYPE_FIGHT)
    End If
    
    If UserList(AttackerIndex).Invent.HerramientaEqpObjIndex = 875 And RandomNumber(1, 10) < 3 Then
        UserList(VictimIndex).flags.Envenenado = 1
        Call SendData(SendTarget.ToIndex, VictimIndex, 0, "||" & UserList(AttackerIndex).name & " te enveneno con su golpe" & FONTTYPE_FIGHT)
    End If
