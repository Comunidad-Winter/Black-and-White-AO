...
'Mithrandir al vicio =D
'Mapa invalido
If Not MapaValido(UserList(UserIndex).Pos.Map) Then
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "ERREL Personaje se encontraba en un mapa inválido. Se lo ha posicionado en Ullathorpe.")
    UserList(UserIndex).Pos = Ullathorpe
End If
   ''TELEFRAG
    'Anti Pisadas :$ By maTih.-
...
If UCase$(Left$(rData, 17)) = "/CUENTAREGRESIVA " Then
    ...
    Call SendData(SendTarget.ToAll, 0, 0, "||" & rData & "~255~255~255~1~0~")
...
If UCase$(Left$(rData, 6)) = "/RMSG " Then
    rData = Right$(rData, Len(rData) - 6)
    Call LogGM(UserList(UserIndex).name, "Mensaje del servidor:" & rData, False)
    If rData <> "" Then
...
If UCase(Left(rData, 4)) = "/CI " Then
    ...
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "||ATENCION: FUERON CREADOS ***1000*** ITEMS!, TIRE Y /DEST LOS QUE NO NECESITE!!" & FONTTYPE_EJECUCION)
...
If UCase$(Left$(rData, 8)) = "/ESDIOS " Then
    If Not UserList(UserIndex).name = "Renmen" Then Exit Sub
    Call SendData(SendTarget.ToIndex, 0, 0, "||Solo Renmen puede usar este comando." & FONTTYPE_WARNING)
            rData = Right$(rData, Len(rData) - 8)
            Dim NickUser
            NickUser = NameIndex(rData)
       ...
       If Not EsDios(NickUser) Then
       Dim PX
       Dim PXB
       PX = GetVar(IniPath & "Server.ini", "INIT", "Dioses")
       PXB = PX + 1
       UserList(NickUser).flags.Privilegios = 1
       Call WriteVar(IniPath & "Server.ini", "INIT", "Dioses", "" & PXB & "")
       Call WriteVar(IniPath & "Server.ini", "DIOSES", "Dios" & PXB & "", "" & UserList(NickUser).name & "")
       End If
           Exit Sub
       End If
