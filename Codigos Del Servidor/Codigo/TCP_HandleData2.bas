...
Procesado = True 'ver al final del sub
    ...
    Exit Sub
    End If
    If MapInfo(100).NumUsers >= 2 Then
    Call SendData(ToIndex, UserIndex, 0, "||¡Ya hay un reto!." & FONTTYPE_TALK)
    Exit Sub
    End If
    ...
    End If
    Else
    Call SendData(ToIndex, UserList(UserIndex).flags.TargetUser, 0, "|| " & UserList(UserIndex).name & " te ha retado, si quieres aceptar, cliquealo y escribe /DUELO." & FONTTYPE_TALK)
    Call SendData(ToIndex, UserIndex, 0, "||Has retado a " & UserList(UserList(UserIndex).flags.TargetUser).name & FONTTYPE_TALK)
    ...
    Exit Sub
           ...
           UserList(UserIndex).flags.Meditando = Not UserList(UserIndex).flags.Meditando
            ...
            If UserList(UserIndex).flags.Meditando Then
                ...
                If UserList(UserIndex).Faccion.ArmadaReal Then
                    Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "CFF" & UserList(UserIndex).Char.CharIndex & "," & ParticulaId.MEDITARARMADA)
                    UserList(UserIndex).Char.Particula = ParticulaId.MEDITARARMADA
                ElseIf UserList(UserIndex).Faccion.FuerzasCaos Then
                    Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "CFF" & UserList(UserIndex).Char.CharIndex & "," & ParticulaId.MEDITARCAOS)
                    UserList(UserIndex).Char.Particula = ParticulaId.MEDITARCAOS
                End If
                End If
                
                 If UserList(UserIndex).flags.Privilegios = PlayerType.Dios Then
                    Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "CFF" & UserList(UserIndex).Char.CharIndex & "," & ParticulaId.PARTICLEGM)
                    UserList(UserIndex).Char.Particula = ParticulaId.PARTICLEGM
        ...
        Case "*" 'al empezar un texto con * si sos conse, semi o dios hablas en evento
rData = Right$(rData, Len(rData) - 2)
If UserList(UserIndex).flags.Privilegios >= 0 Then
Call SendData(SendTarget.ToAll, 0, 0, "|| " & UserList(UserIndex).name & "> " & rData & FONTTYPE_HABLAEVENTO)
Exit Sub
End If
    End Select
    ...
    Select Case UCase$(Left$(rData, 3))
       Case "/GM"
       Call SendData(SendTarget.toadmins, 0, 0, "||El usuario " & UserList(UserIndex).name & " a enviado /GM" & FONTTYPE_CENTINELA)
