...
If Hechizos(Spell).SubeHP = 2 Then
        ...
        Call SendData(SendTarget.ToNPCArea, TargetNPC, Npclist(TargetNPC).Pos.Map, "CFX" & Npclist(TargetNPC).Char.CharIndex & "," & Hechizos(Spell).FXgrh & "," & Hechizos(Spell).loops)
                Call SendData(SendTarget.ToNPCArea, TargetNPC, Npclist(TargetNPC).Pos.Map, "CFF" & Npclist(TargetNPC).Char.CharIndex & "," & Hechizos(Spell).PART & "," & Hechizos(Spell).loops)
...
On Error Resume Next
    ...
    If TieneObjetos(872, 1, UserIndex) And UserList(UserIndex).Invent.HerramientaEqpObjIndex = 872 Then Exit Sub
...
If UserList(UserIndex).flags.Muerto = 0 Then
    ...
    If Hechizos(HechizoIndex).NeedAnillo > 0 Then
            If UserList(UserIndex).Invent.HerramientaEqpObjIndex > 0 Then
               If ObjData(UserList(UserIndex).Invent.HerramientaEqpObjIndex).AnilloPower < Hechizos(HechizoIndex).NeedAnillo Then
                    Call SendData(SendTarget.ToIndex, UserIndex, 0, "||No puedes lanzar este hechizo sin Anillo Epectral." & FONTTYPE_INFO)
                    PuedeLanzar = False
                    Exit Function
                End If
            Else
            ...
            For TempY = PosCasteadaY - 8 To PosCasteadaY + 8
                If InMapBounds(PosCasteadaM, TempX, TempY) Then
                    If MapData(PosCasteadaM, TempX, TempY).UserIndex > 0 Then
                        ...
                        Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "CFF" & UserList(MapData(PosCasteadaM, TempX, TempY).UserIndex).Char.CharIndex & "," & Hechizos(H).PART & "," & Hechizos(H).loops)
                        End If
                    End If
                End If
            Next TempY
...
ElseIf Hechizos(hIndex).SubeHP = 2 Then
    ...
    If UserList(UserIndex).flags.Metamorfosis = 1 Then daño = daño * 2 'Metamorfosis
...
Sub InfoHechizo(ByVal UserIndex As Integer)
    ...
    If UserList(UserIndex).flags.TargetUser > 0 Then
        Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "CFX" & UserList(UserList(UserIndex).flags.TargetUser).Char.CharIndex & "," & Hechizos(H).FXgrh & "," & Hechizos(H).loops)
                        Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "CFF" & UserList(UserList(UserIndex).flags.TargetUser).Char.CharIndex & "," & Hechizos(H).PART & "," & Hechizos(H).loops)
    ...
    ElseIf UserList(UserIndex).flags.TargetNPC > 0 Then
        Call SendData(SendTarget.ToNPCArea, UserList(UserIndex).flags.TargetNPC, Npclist(UserList(UserIndex).flags.TargetNPC).Pos.Map, "CFX" & Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex & "," & Hechizos(H).FXgrh & "," & Hechizos(H).loops)
                        Call SendData(SendTarget.ToNPCArea, UserList(UserIndex).flags.TargetNPC, Npclist(UserList(UserIndex).flags.TargetNPC).Pos.Map, "CFF" & Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex & "," & Hechizos(H).PART & "," & Hechizos(H).loops)
...
Function PuedeLanzable(ByVal UserIndex As Integer, ByVal HechizoIndex As Integer) As Boolean
...
End Function
Sub HechizoEfectoLanzable(UserIndex As Integer)
    'Mithrandir =)
    Dim H As Integer
    H = UserList(UserIndex).Stats.UserHechizos(UserList(UserIndex).flags.Hechizo)
    ...
    'FX y Wav
    Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "CFX" & UserList(UserIndex).Char.CharIndex & "," & Hechizos(H).FXgrh & "," & Hechizos(H).loops)
    ...
    Call SubirSkill(UserIndex, Magia)
    'If Hechizos(uh).Resis = 1 Then Call SubirSkill(UserList(UserIndex).Flags.TargetUser, Resis)
    UserList(UserIndex).Stats.MinMAN = UserList(UserIndex).Stats.MinMAN - Hechizos(H).ManaRequerido
    If UserList(UserIndex).Stats.MinMAN < 0 Then UserList(UserIndex).Stats.MinMAN = 0
    UserList(UserIndex).Stats.MinSta = UserList(UserIndex).Stats.MinSta - Hechizos(H).StaRequerido
    If UserList(UserIndex).Stats.MinSta < 0 Then UserList(UserIndex).Stats.MinSta = 0
    Call SendUserStatsBox(UserIndex)
    'Mithrandir =)
End Sub
 
Sub HechizoAutolanzable(UserIndex As Integer)
Dim H As Integer
'Mithrandir =)
H = UserList(UserIndex).Stats.UserHechizos(UserList(UserIndex).flags.Hechizo)
If Not IntervaloPermiteLanzarSpell(UserIndex) Then Exit Sub
 
    If PuedeLanzable(UserIndex, H) Then
    ...
    'Certero
        If Hechizos(H).GolpeCertero = 1 Then
            ...
            UserList(UserIndex).flags.EfectoTiene = 1
        End If
       
        Call HechizoEfectoLanzable(UserIndex)
    End If
'Mithrandir =)
End Sub
